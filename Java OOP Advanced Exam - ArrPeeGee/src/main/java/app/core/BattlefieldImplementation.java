        ...
        try {
            ...
            for (String name : participantNames){
                if (this.participants.containsKey(name)){
                    ...
                    System.out.println(
                            String.format("%s is not on the battlefield. %s failed."
                                    , name, actionName));
                    return;
                }
            }
           Action action = this.actionFactory.create(actionName, actionParticipants.stream()
                    .map(Targetable::getName).toArray(String[]::new));
    ...
    private void checkForDeadParticipants(){
        ...
        for (String name : this.participants.keySet()) {
            if (!this.participants.get(name).isAlive()){
                this.writer.writeLine(
                        String.format("%s has been removed from the battlefield.", name));
            }else {
